...
public class FolderController {
    ...
    public ResponseEntity<?> createFolder(@RequestBody @Valid RequestCreateDto requestCreateDto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(folderService.createFolder(requestCreateDto));
    }

    @DeleteMapping("/api/v1/folder/delete/{folderId}")
    public ResponseEntity<?> deleteFolder(@PathVariable("folderId") Long folderId) {
        return ResponseEntity.status(HttpStatus.OK).body(folderService.deleteFolder(folderId));
    }
